...
class SassInstallerTest < ActiveSupport::TestCase
  ...
  test "sass installer" do
    with_new_rails_app do
      out, _err = run_installer

      assert File.exist?("app/assets/stylesheets/application.sass.scss")

      assert_match %r{STUBBED yarn add.* sass\s}, out
  ...
  test "sass installer with Bun" do
    with_new_rails_app do
      stub_bins("bun")

      out, _err = run_installer

      assert_match %r{STUBBED bun add.* sass\s}, out
